// Google Apps Script for YesGrant Application - WITH CHATBOT LEADS
...
// Sheet names - Updated to match user's existing sheets
const FORM_SHEET_NAME = 'form submission';  // Changed from 'Form Submissions' 
const SUCCESS_STORIES_SHEET_NAME = 'success stories';  // Changed from 'Success Stories'
const CHATBOT_LEADS_SHEET_NAME = 'Chatbot Leads';  // NEW sheet for chatbot data
...
// Manual test function for form submission
...
// Manual test function for chatbot lead
function testChatbotLead() {
  const testData = {
    ...
    'questionsAsked': 'Asked about scholarships and application process',
    'interestLevel': 'High',
    'sessionDuration': '5 minutes'
  };
  
  console.log('Testing chatbot lead with data:', testData);
  const result = submitChatbotLead(testData);
  console.log('Chatbot lead test result:', result);
  return result;
}
...
function setupSheets() {
  try {
    ...
    if (successSheet.getLastRow() === 0) {
      ...
      // Format header row
      const headerRange = successSheet.getRange(1, 1, 1, headers.length);
    ...
    if (formSheet.getLastRow() === 0) {
      ...
      // Format header row
      const headerRange = formSheet.getRange(1, 1, 1, headers.length);
      headerRange.setBackground('#4CAF50');
      headerRange.setFontColor('white');
      headerRange.setFontWeight('bold');
    }
    ...
    if (chatbotSheet.getLastRow() === 0) {
      ...
      // Format header row
      const headerRange = chatbotSheet.getRange(1, 1, 1, headers.length);
      headerRange.setBackground('#FF9800');
      headerRange.setFontColor('white');
      headerRange.setFontWeight('bold');
    }
